...
const SetList: React.FC<SetListProps> = ({ accessToken, refreshToken, sets }) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (refreshResponse.ok) {
          ...
          const secondTry = await deleteSetAPI(textResponseJSON.accessToken, id)
          if (!secondTry.ok) {
  ...
  return (
    <>
    {setsList.map((set) => {
      return <SetCard key={set.key} id={set.id} name={set.name} author={set.author} deleteSet={deleteSet} />
    })}
    </>
  )
}
