...
const Account: React.FC<AccountProps> = ({ accessToken, refreshToken, username }): React.JSX.Element => {
  const form = useForm({
    mode: 'uncontrolled',
    initialValues: {
      username: ''
    ...
    validate: {
      username: (value: string) => (value !== '' && (/^[a-zA-Z0-9_-]+$/.test(value)) ? null : 'Usernames can only contain letters, numbers, underscores, and dashes')
    }
  })
    ...
    try {
      ...
      if (statusCode === 401) {
        ...
        if (response.ok) {
          ...
          // eslint-disable-next-line no-void
          const statusCodeRetry = await updateUsername(textResponseJSON.accessToken, param)
    ...
    <>
      <div className='mt-3 flex justify-center items-center flex-col gap-3 mx-5'>
        ...
        <Card className='w-full' shadow="sm" padding="lg" radius="md" withBorder>
          ...
          <form onSubmit={form.onSubmit(async (values: { username: string }) => {
        ...
        <a href='/api/signout' className='button px-6 py-3 bg-teal-500 rounded'>Sign Out</a>
      </div>
    </>
  )
}
