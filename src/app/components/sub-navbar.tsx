...
const SubNavbar: React.FC<SubNavbarProps> = ({ session }): React.JSX.Element => {
  ...
  const regularMenu = (): React.JSX.Element => {
    return (
        <div>
          {lg && <div className='flex items-center justify-between width-full p-5 bg-teal-400 text-white text-lg'>
        ...
        <div id='dropdown' className='dropdown-hide flex-col bg-teal-400 pb-5'>
            <div className='pl-5'>
                <div className='text-md'>
                    ...
                    { session && <a href="/sets" className="block mt-3 lg:inline-block lg:mt-0 text-teal-200 hover:text-white mr-4">
                        Sets
                    </a>}
                </div>
                {session && <div>
